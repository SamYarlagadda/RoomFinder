    ...
    try:
        for filename in os.listdir(directory):
            if filename.endswith(".csv"):
                ...
                df = df[df['Location'].str.contains('FENS', na=False)]  # Select only rows where 'Location' contains 'CKB'
        ...
        with open('fens-schedule.html', 'w') as f:
            f.write(html)  # Write the HTML to a file
        webbrowser.open('fens-schedule.html', new=2)  # Open the HTML file in a new browser window
