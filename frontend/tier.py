    ...
    try:
        for filename in os.listdir(directory):
            if filename.endswith(".csv"):
                ...
                df = df[df['Location'].str.contains('TIER', na=False)]  # Select only rows where 'Location' contains 'CKB'
        ...
        with open('tier-schedule.html', 'w') as f:
            f.write(html)  # Write the HTML to a file
        webbrowser.open('tier-schedule.html', new=2)  # Open the HTML file in a new browser window
